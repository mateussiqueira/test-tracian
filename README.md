...
Um aplicativo Flutter para visualização hierárquica de ativos de empresas.

## Características

- Visualização em árvore de ativos e componentes
- Organização por localizações
- Filtros por texto, energia e status crítico
...
```bash
git clone https://github.com/seu-usuario/asset_tree.git
cd asset_tree
```
...
1. **Tela Inicial**
   ...
   - Clique em uma empresa para ver suas localizações

2. **Tela de Localizações**

   - Mostra as localizações da empresa selecionada
   - Expanda uma localização para ver seus ativos
   - Use os filtros para encontrar ativos específicos

3. **Árvore de Ativos**
   - Visualize a hierarquia de ativos e componentes
   - Expanda/colapse nós para navegar
   - Use os filtros para focar em ativos específicos

## Filtros

- **Busca por Texto**: Filtra ativos por nome
- **Sensores de Energia**: Mostra apenas ativos com sensores de energia
- **Status Crítico**: Mostra apenas ativos com status crítico
